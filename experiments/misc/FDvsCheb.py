...
#%%
# N vs Error
N = 2 / np.array([2**i for i in range(4, 10)])
plt.plot(N[::-1], cheb_err_rel[::-1], 'b-o', label="Cheb")
plt.plot(N[::-1], fd_err_rel[::-1], 'r-x', label="FD")
plt.plot(N[::-1], 1e4*N[::-1]**2, label=r"$O(h^2)$")
plt.xscale('log')
plt.yscale('log')
plt.xlabel(r"$h$")
plt.ylabel("Error")
plt.xticks(N, map(lambda N: '2/' + str(int(N)), 2/(N)))
plt.grid(True)
plt.legend()
plt.show()
